# Daily GitHub Activity (2026-09-20)

<!-- nav -->
[← 2026-09-19](archive/2026/09/19.md)
<!-- /nav -->

Today's public activity from users I follow (plus anyone listed in `custom_users.txt`), updated every 15 minutes.

## Today's Activity

### [CrackerCat](https://github.com/CrackerCat)
- 🍴 👤 [CrackerCat](https://github.com/CrackerCat) Forked [MSNightmare/BigDiskBuster](https://github.com/MSNightmare/BigDiskBuster) to [CrackerCat/BigDiskBuster](https://github.com/CrackerCat/BigDiskBuster)
  > Windows Defender Update Denial of Service Vulnerability
- 🍴 👤 [CrackerCat](https://github.com/CrackerCat) Forked [tearhacker/zygisk_frida_Packet_tcp](https://github.com/tearhacker/zygisk_frida_Packet_tcp) to [CrackerCat/zygisk_frida_Packet_tcp](https://github.com/CrackerCat/zygisk_frida_Packet_tcp)
  > 一款模块抓包工具，泪心自用工具

### [Razzue](https://github.com/Razzue)
- 🌟 👤 [Razzue](https://github.com/Razzue) Starred [FascinatinC/Wow-Dumper-fix](https://github.com/FascinatinC/Wow-Dumper-fix)
  > A simple wow offset dumper

### [dword64](https://github.com/dword64)
- 🌟 👤 [dword64](https://github.com/dword64) Starred [noahware/kernemul](https://github.com/noahware/kernemul)
  > Windows kernel driver and usermode app emulator for x86-64 and ARM64 targets.

### [ggoodman](https://github.com/ggoodman)
- 🌟 👤 [ggoodman](https://github.com/ggoodman) Starred [agent-substrate/env](https://github.com/agent-substrate/env)
  > An agentic environment abstraction for Agent Substrate

### [jevinskie](https://github.com/jevinskie)
- 🌟 👤 [jevinskie](https://github.com/jevinskie) Starred [mowisec/frida_patches_srd](https://github.com/mowisec/frida_patches_srd)
  > Kernel and TXM patches allowing Frida to run on iOS 27 SRD
- 🌟 👤 [jevinskie](https://github.com/jevinskie) Starred [mowisec/imessage](https://github.com/mowisec/imessage)
- 🌟 👤 [jevinskie](https://github.com/jevinskie) Starred [mowisec/insert_kext](https://github.com/mowisec/insert_kext)
  > Patch an open-source kext into a closed-source iOS kernelcache binary.
- 🌟 👤 [jevinskie](https://github.com/jevinskie) Starred [mowisec/macos-to-ios](https://github.com/mowisec/macos-to-ios)
- 🌟 👤 [jevinskie](https://github.com/jevinskie) Starred [MikeM64/ps3-sb-uart-loader](https://github.com/MikeM64/ps3-sb-uart-loader)
  > Bare-metal Southbridge UART ELF Loader
- 🌟 👤 [jevinskie](https://github.com/jevinskie) Starred [pavelfpl/gidel_hawkeye_pcie_design](https://github.com/pavelfpl/gidel_hawkeye_pcie_design)
  > This repository contains basic FPGA design and Linux PCIe driver for Gidel HawkEye 20G board
- 🌟 👤 [jevinskie](https://github.com/jevinskie) Starred [pavelfpl/altera_msgdma_st](https://github.com/pavelfpl/altera_msgdma_st)
  > Altera mSGDMA DMA kernel driver with Altera Streaming ST® protocol support

### [mrexodia](https://github.com/mrexodia)
- 🍴 👤 [mrexodia](https://github.com/mrexodia) Forked [LittleBigRefresh/scetool](https://github.com/LittleBigRefresh/scetool) to [mrexodia/scetool](https://github.com/mrexodia/scetool)
  > Modification around SCETool to turn it into a library.
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [xangel82/Athena-Engine](https://github.com/xangel82/Athena-Engine)
  > LLM Engine for DGX Spark 
- 🌟 👤 [mrexodia](https://github.com/mrexodia) Starred [jaredpalmer/kev](https://github.com/jaredpalmer/kev)
  > tiny Jev-like family of decision models built on top of Qwen3.5 you can train and run on your own

### [niieani](https://github.com/niieani)
- 🍴 👤 [niieani](https://github.com/niieani) Forked [imputnet/helium](https://github.com/imputnet/helium) to [niieani/helium](https://github.com/niieani/helium)
  > Private, fast, and honest web browser

### [seifreed](https://github.com/seifreed)
- 🚀 👤 [seifreed](https://github.com/seifreed) Made [seifreed/netflixcli](https://github.com/seifreed/netflixcli) public
  > Unofficial, agent-friendly command-line client for netflix.com


<!-- nav -->
[← 2026-09-19](archive/2026/09/19.md)
<!-- /nav -->

---
*Last updated at 2026-09-20 23:58:25 UTC*
<!-- archive-link -->
*Historical records are stored in the [`archive`](archive/2026/09) directory.*
<!-- /archive-link -->
